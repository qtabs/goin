...
class Model():
	...
	def training_log(self, lossHistory, epoch0, batch_res, savename, benchmarks):
		...
		plotvec = dict()
		for key in ['mse', 'kol', 'ce', 'ct_ac', 'ct_ce']:
			plotvec[key] = dict()
			for val in ['avg', 'sem']:
				plotvec[key][val] =  [perf[mod][key][val] for mod in ['LI', 'coin', 'gru']]

		colours, xticks = ['tab:purple', 'tab:green', 'tab:gray'], [0, 1, 2]
		ax_mse.bar(xticks, plotvec['mse']['avg'], yerr=plotvec['mse']['sem'], color=colours)
		ax_mse.set_xticks(xticks)
		ax_mse.set_xticklabels(['Leak-Int', 'COIN', 'GRU'])
		ax_mse.set_ylabel('mse')
		ax_kol.bar(xticks, plotvec['kol']['avg'], yerr=plotvec['kol']['sem'], color=colours)
		ax_kol.set_xticks(xticks)
		ax_kol.set_xticklabels(['Leak-Int', 'COIN', 'GRU'])
		ax_kol.set_ylabel('KS-stat')
		
		ax_cacc.bar(xticks, plotvec['ct_ac']['avg'], yerr=plotvec['ct_ac']['sem'], color=colours)
		ax_cacc.set_xticks(xticks)
		ax_cacc.set_xticklabels(['E[beta]', 'COIN', 'GRU'])
		ax_cacc.set_ylabel('ctx accuracy')
		ax_cce.bar( xticks, plotvec['ct_ce']['avg'],  yerr=plotvec['ct_ce']['sem'],  color=colours)
		ax_cce.set_xticks(xticks)
		ax_cce.set_xticklabels(['E[beta]', 'COIN', 'GRU'])
		ax_cce.set_ylabel('<ctx lopg>')
